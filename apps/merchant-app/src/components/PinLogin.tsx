...
export function PinLogin({ onSuccess }: Props) {
  ...
  const handleLogin = async () => {
    if (!merchantId.trim() || !pin.trim()) return;
    setLoading(true);
    setError(null);
    try {
      await login(merchantId.trim(), pin.trim());
      onSuccess();
    } catch (e: any) {
      setError(e?.data?.message ?? "Login failed. Check your credentials.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <View className="flex-1 bg-bg items-center justify-center px-8">
      ...
      <Text className="text-ink-muted text-base mb-10">Merchant Login</Text>
