...
export class AdvanceService {
  ...
  async checkEligibility(merchantId: string, transactionId: string): Promise<AdvanceOffer> {
    ...
    const maxAdvance = Math.round(merchant.trailing30dVolumeCents * (maxPercent / 100));
    const advanceAmount = Math.min(advanceBase, maxAdvance);
    const advanceFee = Math.round(advanceAmount * (advanceFeePercent / 100));

    return { eligible: true, advanceAmount, advanceFee, feeRate: advanceFeePercent };
  }
  ...
  async execute(merchantId: string, transactionId: string): Promise<Advance> {
    ...
    try {
      ...
      const advance = queryRunner.manager.create(Advance, {
        merchantId,
        transactionId,
        originalAmount: offer.advanceAmount,
        advanceAmount: offer.advanceAmount,
        advanceFee: offer.advanceFee,
        status: "OUTSTANDING",
      });
  ...
  async settleFromCardSettlement(
    ...
    const advance = await this.advanceRepo.findOne({
      where: { transactionId, status: "OUTSTANDING" },
    });
    ...
    try {
      ...
      const remainder = settledAmount - advance.originalAmount - advance.advanceFee;
      if (remainder > 0) {
        const wallet = await this.walletService.findByMerchantId(advance.merchantId);
        await this.walletService.credit(
          queryRunner,
          wallet.id,
          remainder,
